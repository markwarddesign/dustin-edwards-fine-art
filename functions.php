...
function dedwards_render_collection_section( $attributes ) {
    ...
    if ( $is_archive ) {
        ...
        // Two queries so in-progress always renders last, but nothing is hidden
        $base_tax = $selected_cat_slug ? array(
            'relation' => 'AND',
            array( 'taxonomy' => 'work_category', 'field' => 'slug', 'terms' => $selected_cat_slug ),
            array( 'taxonomy' => 'work_category', 'field' => 'slug', 'terms' => 'in-progress', 'operator' => 'NOT IN' ),
        ) : array(
            array( 'taxonomy' => 'work_category', 'field' => 'slug', 'terms' => 'in-progress', 'operator' => 'NOT IN' ),
        );

        $regular_works = new WP_Query( array(
            'post_type'      => 'work',
            'posts_per_page' => -1,
            'post_status'    => 'publish',
            'tax_query'      => $base_tax,
        ...
        $progress_tax = $selected_cat_slug ? array(
            'relation' => 'AND',
            array( 'taxonomy' => 'work_category', 'field' => 'slug', 'terms' => 'in-progress' ),
            array( 'taxonomy' => 'work_category', 'field' => 'slug', 'terms' => $selected_cat_slug ),
        ) : array(
            array( 'taxonomy' => 'work_category', 'field' => 'slug', 'terms' => 'in-progress' ),
        );
        ...
        <div class="pt-32 md:pt-48 px-6 md:px-12 pb-12 bg-stone-50 min-h-screen">
            <div class="max-w-7xl mx-auto">
                ...
                <p class="font-serif italic text-stone-500 mb-12 max-w-2xl"><?php echo esc_html( $subheading ); ?></p>

                <!-- Category Filter -->
                <div class="flex flex-wrap gap-2 mb-16 border-b border-stone-200 pb-8">
                    <a href="<?php echo esc_url( $archive_url ); ?>"
                       class="text-[10px] uppercase tracking-[0.2em] px-4 py-2 border transition-colors duration-200 <?php echo empty( $selected_cat_slug ) ? 'bg-stone-900 text-white border-stone-900' : 'border-stone-300 text-stone-500 hover:border-stone-900 hover:text-stone-900'; ?>">
                        All
                    </a>
                    <?php if ( ! is_wp_error( $filter_cats ) ) : foreach ( $filter_cats as $fc ) : ?>
                    <a href="<?php echo esc_url( add_query_arg( 'cat', $fc->slug, $archive_url ) ); ?>"
                       class="text-[10px] uppercase tracking-[0.2em] px-4 py-2 border transition-colors duration-200 <?php echo $selected_cat_slug === $fc->slug ? 'bg-stone-900 text-white border-stone-900' : 'border-stone-300 text-stone-500 hover:border-stone-900 hover:text-stone-900'; ?>">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-y-24 gap-x-12">
                    <?php
                    $has_any = $regular_works->have_posts() || $progress_works->have_posts();
                    if ( ! $has_any ) : ?>
                        <p class="col-span-full text-center text-stone-500">No works found in this category.</p>
                    <?php endif; ?>

                    <?php // Regular works first
                    while ( $regular_works->have_posts() ) : $regular_works->the_post(); ?>
                        <?php
                    ...
                    <?php // In-progress always at end
                    while ( $progress_works->have_posts() ) : $progress_works->the_post(); ?>
                        <?php
                        $material        = get_post_meta( get_the_ID(), '_work_material', true ) ?: 'Bronze';
                        $featured_image  = get_the_post_thumbnail_url( get_the_ID(), 'large' );
                        if ( ! $featured_image ) $featured_image = 'https://images.unsplash.com/photo-1628607153673-455b550117d9?q=80&w=1500&auto=format&fit=crop';
                ...
                <div class="mt-48 pt-20 text-center max-w-3xl mx-auto border-t border-stone-200">
                    <h2 class="font-serif italic text-3xl md:text-4xl text-stone-900 mb-6">Commission a Unique Piece</h2>
                    <p class="font-serif text-lg text-stone-600 mb-10 leading-relaxed">Each bronze sculpture is a labor of love, crafted to capture the essence of the American West. I welcome commissions for custom works that bring your vision to life in timeless bronze.</p>
                    <a href="<?php echo esc_url( home_url( '/inquire' ) ); ?>" class="inline-block bg-stone-900 text-white py-5 px-12 text-[11px] uppercase tracking-[0.3em] hover:bg-bronze-700 transition-all duration-300 font-medium">Start a Conversation</a>
                </div>
            </div>
        </div>
        <?php
    ...
    ob_start();
    ?>
    <div class="px-4 md:px-12 pb-32 bg-bronze-50">
        ...
        <?php if ( $show_view_all && $works->found_posts > $posts_per_page ) : ?>
